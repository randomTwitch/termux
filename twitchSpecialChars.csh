...
#	echo '<'$$'>';
	...
	if ( $s == 0 ) then;
	(termux-toast "$f found in 1stTwitchRed.ascii";&);&; exit 2;
	endif

	grep /$f/ $p2 >& /dev/null;set s=$status;
	if ( $s == 0 ) then;
	(termux-toast "$f found in 2ndTwitchViolet.ascii";&);&; exit 2;
	endif

	grep /$f/ $p3 >& /dev/null; set s=$status;
	if ( $s == 0 ) then;
	(termux-toast "$f found in 3rdTwitchLblue.ascii";&);&; exit 2;
	endif

	grep /$f/ $p4 >& /dev/null; set s=$status;
	if ( $s == 0 ) then;
	(termux-toast "$f found in 4thTwitchWhite.ascii";&);&; exit 2;
	endif
