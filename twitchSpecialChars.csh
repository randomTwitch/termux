...
#	echo '<'$$'>';
	...
	if ( $s == 0 ) then;
	(termux-toast "$f found in 1stTwitchRed.ascii";&);&; sleep 2s; wait; exit 2;
	endif

	grep /$f/ $p2 >& /dev/null;set s=$status;
	if ( $s == 0 ) then;
	(termux-toast "$f found in 2ndTwitchViolet.ascii";&);&; sleep 2s; wait; exit 2;
	endif

	grep /$f/ $p3 >& /dev/null; set s=$status;
	if ( $s == 0 ) then;
	(termux-toast "$f found in 3rdTwitchLblue.ascii";&);&; sleep 2s; wait; exit 2;
	endif

	grep /$f/ $p4 >& /dev/null; set s=$status;
	if ( $s == 0 ) then;
	(termux-toast "$f found in 4thTwitchWhite.ascii";&);&; sleep 2s; wait; exit 2;
	endif

	(rm $1;&);&;
	(echo "https://m.twitch.tv/$f/home" >>& $p4;&);&;
	(echo -n "https://m.twitch.tv/$f/home#" >>& $p0;&);&;
	unset e p00 p0 p1 p2 p3 p4 mgs_error mgs_exit mgs_pass s f; wait; exit 1; 
## special chars twitch app
